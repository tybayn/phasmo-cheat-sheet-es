...
var user_settings = {"num_evidences":"3","cust_num_evidences":"3","cust_hunt_length":"3","ghost_modifier":2,"volume":50,"mute_timer_toggle":0,"mute_timer_countdown":0,"timer_count_up":0,"timer_split":1,"offset":0.0,"sound_type":0,"speed_logic_type":0,"bpm":0,"domo_side":0,"priority_sort":0,"map":"6 Tanglewood Drive","theme":"Por defecto"}
...
function closeMenu(){
    ...
    if(mquery.matches){
        document.getElementById("menu").style.marginBottom = is_c ? "-755px" : "-700px";
...
function showMaps(forceOpen = false, forceClose = false){
    mquery = window.matchMedia("screen and (pointer: coarse) and (max-device-width: 600px)")
    if (document.getElementById("maps_box").style.left == (mquery.matches ? "calc(-100% + 60px)" : "-346px") && !forceClose){
        ...
        document.getElementById("maps_box").style.left = (mquery.matches ? "0px" : "238px")
        document.getElementById("maps_box").style.width = (mquery.matches ? "calc(100% - 100px)" : "calc(100% - 307px)")
    }
    else if(!forceOpen) {
        document.getElementById("maps_box").style.width = (mquery.matches ? "calc(100% - 100px)" : "556px")
        document.getElementById("maps_box").style.left = (mquery.matches ? "calc(-100% + 60px)" : "-346px")
...
function flashMode(){
    ...
    var mode_text = {"-1":"Personalizada","0":"Apocalipsis III","1":"Demencia","2":"Pesadilla","3":"Profesional","3I":"Intermedia","3A":"Principiante"}[cur_evidence]
    document.getElementById("game_mode").innerHTML = `${mode_text}<span>(${parseInt(cur_evidence)} evidencia)</span>`.replace("-1",document.getElementById("cust_num_evidence").value)
...
function loadSettings(){
    ...
    try{
        ...
        user_settings = {"num_evidences":"3","cust_num_evidences":"3","cust_hunt_length":"3","ghost_modifier":2,"volume":50,"mute_timer_toggle":0,"mute_timer_countdown":0, "timer_count_up":0,"timer_split":1,"offset":0.0,"sound_type":0,"speed_logic_type":0,"bpm_type":0,"bpm":0,"domo_side":0,"priority_sort":0,"map":"6 Tanglewood Drive","theme":"Por defecto"}
    }
...
function showCustom(){
    ...
    if(document.getElementById("num_evidence").value == "-1"){
        if(mquery.matches){
            document.getElementById("menu").style.height="790px";
            if(is_h){
                document.getElementById("menu").style.marginBottom = "-755px";
                $("#domovoi").removeClass("domovoi-custom")
                $("#domovoi").addClass("domovoi-menu-hidden")
            }
            else{
        ...
        if(mquery.matches){
            if(is_h){
                document.getElementById("menu").style.marginBottom = "-700px";
                $("#domovoi").removeClass("domovoi-custom")
                $("#domovoi").addClass("domovoi-menu-hidden")
            }
            else{
            ...
            document.getElementById("menu").style.height="735px";
        }
    }
}
